...
class RNN:
    ...
    def train(self, data, epochs=10):
      ...
      for i in range(epochs):
        while(self.e != 0):
          ...
          if i % 1 == 0:
            print("epoch ", i, " // smooth loss: ", round(smooth_loss,3))
          if i % 5 == 0:
    ...
    def compute_grads(self, X, Y, p, a, h):
      ...
      for t in range(self.seq_length):
        grad_w += np.dot(grad_a[t].reshape(self.m,1), self.h_prev.T)
           
      grad_u = np.zeros((self.m, self.k))
      for t in range(self.seq_length):
        xt = X[:,t].reshape(self.k, 1)
        grad_u += np.dot(grad_a[t].reshape(self.m,1), xt.T) # for t = 0 this takes the last X
    ...
    def check_gradients(self, data, h_param = 1e-5):
      X, Y = self.get_matrices(data)
      p, a, h = self.evaluate(X)
      self.compute_grads(X, Y, p, a, h)
